...
| **應用部署** | [deployment.yaml](charts/spring-boot-demo/templates/deployment.yaml) | Spring Boot 主程式與 Tunnel 側車容器 (SSH 反向隧道)。 |
...
| 技術 | 角色 | 功能 |
| :--- | :--- | :--- |
| **Docker** | 集裝箱（貨櫃） | **包裝與隔離**。將 Spring Boot 主程式與 JRE 封裝為不可變映像檔，確保各環境運行一致。 |
...
│   │   │ (ClusterIP 內網入口)   │ ──(轉發流量)─► │ (快取資料庫 Pod 實體)   │   │       │
