...
│                                                                                    │
│   ┌────────────────── Pod (共享同一個網路空間 localhost) ───────────────┐           │
│   │                                                                    │           │
│   │  [spring-boot-demo 主應用程式] ──(1) 內網連線:6379──► [redis-service]─┼───┐       │
│   │          ▲                                                         │   │       │
│   │          │ (轉發本機流量 127.0.0.1:8080)                            │   ▼       │
│   │          │                                                         │ [Redis Pod]
│   │  [Tunnel 側車容器 (Alpine/SSH)]                                     │(ClusterIP)│
│   │          ▲                                                         │   │       │
│   └──────────┼─────────────────────────────────────────────────────────┘   └───┬───┘
│              │                                                                 │
│              │ (2) 建立 SSH 反向安全隧道 (443 埠)                                 │ (僅限內網存取)
│              ▼                                                                 ▼
│       [Pinggy 公開 HTTPS 網址] ◄─── (瀏覽器存取) ─── [外部用戶]                  [外網不可見]
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```
---
