...
│                                                                                    │
│   ┌────────────────── Pod (共享同一個網路命名空間 localhost) ──────────────┐        │
│   │                                                                        │        │
│   │  [spring-boot-demo 主容器] ──(1) 內網連線:6379──► [redis-service]──────┼───┐    │
│   │         ▲ (監聽 127.0.0.1:8080)                                        │   │    │
│   │         │                                                              │   ▼    │
│   │         │ (2) 本地轉發 (Local Traffic Forwarding)                      │ [Redis]│
│   │         │                                                              │ (6379) │
│   │  [tunnel 側車容器 (Sidecar / SSH)]                                     │        │
│   │         ▲                                                              │   │    │
│   └─────────┼──────────────────────────────────────────────────────────────┘   └───┬┘
│             │                                                                      │
│             │ (3) 建立 SSH 反向隧道連線 (free.pinggy.io:443)                        │ (僅限內網)
│             ▼                                                                      ▼
│     [Pinggy 公開 HTTPS 網址] ◄─── (瀏覽器存取) ─── [外部用戶]                        [外網隔離]
│                                                                                    │
└────────────────────────────────────────────────────────────────────────────────────┘
```
---
